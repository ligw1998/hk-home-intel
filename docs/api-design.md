...
- 默认接口只返回 `is_active = true` 项；LandsD 官方同步会把最新月报中不再出现的同 source 旧项标记为 inactive
- 当前 signal 顺序大致为：`LandsD Pending / LandsD Issued / Recent Pricing / Recent Brochure / SRPE Active / Commercial Launch / Manual Watch / Other Watch`
...
`GET /api/v1/system/overview` 当前还会返回 preflight / data-quality 指标：

- `development_missing_coordinates_count`
- `duplicate_development_name_group_count`
- `active_listing_missing_price_count`
- `commercial_canonical_with_official_artifact_count`

其中 `commercial_canonical_with_official_artifact_count` 用来捕捉历史上可能由商业源先建主身份、但后来已经出现官方文档或 SRPE artifacts 的 development，便于人工复查 source identity。
